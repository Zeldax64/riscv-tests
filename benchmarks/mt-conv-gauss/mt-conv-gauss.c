...
extern void convGauss(int coreid, int ncores, int img_height,int img_width,int kernel_dim, data_t img[IMG_HEIGHT][IMG_WIDTH],data_t_s kernel[KERNEL_DIM*KERNEL_DIM], data_t result[IMG_HEIGHT][IMG_WIDTH]);
...
{	
	...
	convGauss(cid, nc, IMG_HEIGHT, IMG_WIDTH, KERNEL_DIM, input1_data,input2_data, results_data);
